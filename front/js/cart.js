...
//ecoute de la quantité total = prix’
   ...
   function qtyListen() {
    ...
    qtyInput.forEach(function (input) { // pour chaque input quantité qu'on ecoute
      input.addEventListener("change", function (inputevent) { // à chaque changement dans l'input
...
function validation() {
  ...
  let validEmail = validemail(email);
    const validFormu = validFirstname && validlastName && validCity && validaddress && validEmail;
